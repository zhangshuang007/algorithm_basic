 ...
 *
 *  题目
 *  数组arr代表每一个咖啡机冲一杯咖啡的时间，每个咖啡机只能串行的制造咖啡。默认是有序的
...
public class Code06_Coffee {
	...
	// drinks[index...]都想变干净，这是我操心的，
	// washLine表示洗的机器可用时间节点
	// drinks[index...]变干净，最少的时间点返回
	public static int process(int[] drinks, int a, int b, int index, int washLine) {
		// basecase
		if (index == drinks.length - 1) {// 最后一杯咖啡，两个决策 1.洗咖啡杯，2.挥发
			return Math.min(
					Math.max(washLine, drinks[index]) + a// 清洗机器可用时间点washLine=10，最后一杯喝完的时间点是18，那么肯定从18才能开始洗杯子
					, drinks[index] + b);// 喝完最后一杯就可以挥发
		}
		// 剩不止一杯咖啡
		// wash是我当前的咖啡杯，洗完的时间
		int wash = Math.max(washLine, drinks[index]) + a;// 洗index一杯，结束的时间点
		// index+1...变干净的最早时间
		int next1 = process(drinks, a, b, index + 1, wash);
		// index...变干净的最早时间（index开始整体变干净，index自己变干净，以及index之后也得变干净，谁更晚谁就是瓶颈）
