...
public class Code09_NQueens {
	...
	// 请不要超过32皇后问题，位运算优化常数时间，复杂度不变还是N的N次方
	public static int num2(int n) {
		if (n < 1 || n > 32) {
			return 0;
		}
		// 如果你是13皇后问题，limit 最右13个1，其他都是0
		int limit = n == 32 ? -1 : (1 << n) - 1;// -1就是32个1
		return process2(limit, 0, 0, 0);
	}

	// limit 划定了问题的规模-> 固定   比如8皇后 0...0 11111111
	...
	public static int process2(int limit, 
		...
		// ~(colLim | leftDiaLim | rightDiaLim) -> 左侧的一坨1干扰，右侧每个1，可尝试放皇后
		int pos = limit & (~(colLim | leftDiaLim | rightDiaLim));
		int mostRightOne = 0;
		int res = 0;
		while (pos != 0) {
			// 期取出pos中，最右侧的1来，剩下的位置都是0
